...
class AutenticacionServicio:
    """
    Clase que contiene la logica de negocio para la autenticacion de usuarios.
    """
    ...
    @staticmethod
    def crear_token_acceso(data: dict, tiempo_expiracion_token: Optional[timedelta]) -> str:
        """
        Crea un token de acceso.
        Args:
            data (dict): Informacion del usuario.
            tiempo_expiracion_token (Optional[timedelta]): Tiempo de expiracion del token.
        Returns:
            str: Token de acceso.
        """
        ...
        return jwt.encode(para_codificar, settings.SECRET_KEY, algorithm=settings.JWT_ALGORITHM)
    ...
    def verificar_contrasenia(contrasenia_plana, contrasenia_encriptada) -> bool:
        ...
        Args:
            contrasenia_plana (str): Contraseña ingresada.
    ...
    async def autenticar_usuario(self, usuario, password) -> UsuarioEntidad:
        ...
        if not usuario:
            raise AplicacionException("El usuario ingresado es incorrecto", status.HTTP_401_UNAUTHORIZED)
        contrasenia_valida: bool = self.verificar_contrasenia(password, usuario.password)
        if not contrasenia_valida:
            raise AplicacionException("La contraseña ingresada es incorrecta", status.HTTP_401_UNAUTHORIZED)
        # Realiza la comprobacion de si el usuario esta habilitado para ingresar al sistema.
        if not usuario.esta_habilitado:
            raise AplicacionException("El usuario no está habilitado para ingresar al sistema",
                                      status.HTTP_401_UNAUTHORIZED)
        return usuario

    async def iniciar_sesion(self, request: OAuth2PasswordRequestForm) -> IniciarSesionResponse:
        """
        Realiza el inicio de sesion de un usuario.
        Args:
            request (OAuth2PasswordRequestForm): Datos del usuario.
        Returns:
            IniciarSesionResponse: Token de acceso.
        """
        usuario: UsuarioEntidad = await self.autenticar_usuario(request.username, request.password)
        tiempo_expiracion_token: timedelta = timedelta(minutes=settings.JWT_ACCESS_TOKEN_EXPIRE_MINUTES)
