...
class ConfiguracionServicio:
    ...
    async def obtener_todos(self) -> list[ObtenerTodosConfiguracionResponse]:
        ...
        """
        configuraciones: list[ConfiguracionEntidad] = await self._configuracion_repositorio.obtener_todos({
            "estado": base_entidad.ESTADO_ACTIVO
        })
        return [ObtenerTodosConfiguracionResponse(**configuracion.dict()) for configuracion in configuraciones]

    async def actualizar(self, configuracion_id: str, valor: str, usuario_auditoria_id: str) -> str:
        """
        ...
        """
        configuracion = await self._configuracion_repositorio.obtener_por_id(configuracion_id)
        configuracion.valor = valor
        configuracion.registrar_actualizacion(usuario_auditoria_id)
        return await self._configuracion_repositorio.actualizar(configuracion.id, configuracion)
        ...
        for configuracion in configuraciones:
            ...
            # Latitud inicial.
                ...
                if request.latitud_inicial < -90 or request.latitud_inicial > 90:
                    raise AplicacionException("La latitud inicial debe estar entre -90 y 90")
                configuracion_id: str = await self.actualizar(
                    configuracion.id,
                    str(request.latitud_inicial),
                    usuario_auditoria_id
                )
                configuraciones_actualizadas.append(configuracion_id)
            # Longitud inicial
                ...
                if request.longitud_inicial < -180 or request.longitud_inicial > 180:
                    raise AplicacionException("La longitud inicial debe estar entre -180 y 180")
                configuracion_id: str = await self.actualizar(
                    configuracion.id,
            ...
            # Capa base inicial.
                ...
                if request.capa_base_inicial_id:
                    ...
                    if not existe_capa_base:
                        raise AplicacionException("La capa base inicial seleccionada no existe")
                configuracion_id: str = await self.actualizar(
                    configuracion.id,
                    request.capa_base_inicial_id,
                    usuario_auditoria_id
