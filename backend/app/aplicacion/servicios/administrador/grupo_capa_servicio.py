...
class GrupoCapaServicio:
    """
    Clase que contiene la logica de negocio para la administracion de grupos de capas.
    """
    ...
    async def __obtener_grupo_capa_descripcion(self, grupo_capa_id: Optional[str]) -> Optional[str]:
        """
        Obtiene la descripcion de un grupo de capas.
        Args:
            grupo_capa_id (Optional[str]): Identificador del grupo de capas.
        Returns:
            Optional[str]: Descripcion del grupo de capas.
        """
        grupo_capa: GrupoCapaEntidad = await self._grupo_capa_repositorio.obtener_por_filtros({
            "_id": ObjectId(grupo_capa_id),
    ...
    def __obtener_hijos(self, grupo_capa_id: str, grupos_capas: list[GrupoCapaEntidad]) -> \
        ...
        Returns:
            list[ObtenerEstructuraGrupoCapaResponse]: Lista de hijos del grupo de capas.
        """
        hijos: list[ObtenerEstructuraGrupoCapaResponse] = []
        for grupo_capa in grupos_capas:
            if grupo_capa.grupo_capa_id == grupo_capa_id:
                hijos.append(ObtenerEstructuraGrupoCapaResponse(
                    id=grupo_capa.id,
    ...
    def __obtener_estructura_grupos_capas(self, grupos_capas: list[GrupoCapaEntidad]) \
            -> list[ObtenerEstructuraGrupoCapaResponse]:
        """
        ...
        Args:
            grupos_capas (list[GrupoCapaEntidad]): Lista de grupos de capas.
        Returns:
            list[ObtenerEstructuraGrupoCapaResponse]: Lista de grupos de capas.
        """
        raices: list[ObtenerEstructuraGrupoCapaResponse] = []
        for grupo_capa in grupos_capas:
            if grupo_capa.grupo_capa_id is None:
                raices.append(ObtenerEstructuraGrupoCapaResponse(
                    id=grupo_capa.id,
                    label=grupo_capa.nombre,
                    children=self.__obtener_hijos(grupo_capa.id, grupos_capas)
                ))
    ...
    async def obtener_todos(self) -> list[ObtenerTodosGrupoCapaResponse]:
        """
        Obtiene todos los grupos de capas activos.
        Returns:
            list[ObtenerTodosGrupoCapaResponse]: Lista de grupos de capas activos.
        """
        grupos_capas: list[GrupoCapaEntidad] = await self._grupo_capa_repositorio.obtener_todos({
            "estado": ESTADO_ACTIVO
        })
        return [ObtenerTodosGrupoCapaResponse(
            id=grupo_capa.id,
            nombre=grupo_capa.nombre,
            grupo_capa=await self.__obtener_grupo_capa_descripcion(grupo_capa.grupo_capa_id),
    ...
    async def obtener_por_id(self, grupo_capa_id: str) -> ObtenerPorIdGrupoCapaResponse:
        """
        Obtiene una grupo de capas por su identificador.
        Args:
            grupo_capa_id (str): Identificador del grupo de capas.
        Returns:
            ObtenerPorIdGrupoCapaResponse: Capa base.
        """
        grupo_capa: GrupoCapaEntidad = await self._grupo_capa_repositorio.obtener_por_id(grupo_capa_id)
        if not grupo_capa.estado:
            raise AplicacionException("El grupo de capas no existe", status.HTTP_404_NOT_FOUND)
    ...
    async def obtener_estructura(self) -> list[ObtenerEstructuraGrupoCapaResponse]:
        ...
        Returns:
            list[ObtenerEstructuraGrupoCapaResponse]: Lista de grupos de capas.
        Returns:
            list[ObtenerEstructuraGrupoCapaResponse]: Estructura de los grupos de capas.
        """
        grupos_capas: list[GrupoCapaEntidad] = await self._grupo_capa_repositorio.obtener_todos({
            "estado": ESTADO_ACTIVO
        })
        elementos_no_agrupados: list[ObtenerEstructuraGrupoCapaResponse] = self.__obtener_estructura_grupos_capas(
            grupos_capas)
        return [
    ...
    async def crear(self, request: CrearGrupoCapaRequest, usuario_auditoria_id: str) -> str:
        """
        Crea un nuevo grupo de capas.
        Args:
            request (CrearGrupoCapaRequest): Informacion del grupo de capas a crear.
            usuario_auditoria_id (str): Identificador del usuario que crea el grupo de capas.
        Returns:
        ...
        if request.grupo_capa_id:
            existe_grupo_capa: bool = await self._grupo_capa_repositorio.verificar_existencia(request.grupo_capa_id)
            if not existe_grupo_capa:
                raise AplicacionException("El grupo de capas padre no existe", status.HTTP_404_NOT_FOUND)
    ...
    async def actualizar(self, grupo_capa_id: str, request: ActualizarGrupoCapaRequest,
                         usuario_auditoria_id: str) -> str:
        """
        Actualiza un grupo de capas existente.
        Args:
            grupo_capa_id (str): Identificador del grupo de capas.
            request (ActualizarGrupoCapaRequest): Información del grupo de capas a actualizar.
            usuario_auditoria_id (str): Identificador del usuario que actualiza el grupo de capas.
        Returns:
        ...
        if grupo_capa_id == request.grupo_capa_id:
            raise AplicacionException("El grupo de capas padre no puede ser el mismo grupo de capas",
    ...
    async def eliminar(self, grupo_capa_id: str, usuario_auditoria_id: str) -> str:
        """
        Elimina un grupo de capas existente.
        Args:
            grupo_capa_id (str): Identificador del grupo de capas a eliminar.
            usuario_auditoria_id (str): Identificador del usuario que elimina el grupo de capas.
        Returns:
            str: Identificador del grupo de capas eliminado.
        """
        # Se verifica que el grupo de capas exista.
        existe_grupo_capa: bool = await self._grupo_capa_repositorio.verificar_existencia(grupo_capa_id)
        if not existe_grupo_capa:
            raise AplicacionException("El grupo de capas no existe", status.HTTP_404_NOT_FOUND)
        # Se verifica que el grupo de capas no tenga otros grupos de capas internos.
        existe_elementos_internos: bool = await self._grupo_capa_repositorio.verificar_existencia_por_filtros({
            "estado": ESTADO_ACTIVO,
            "grupo_capa_id": grupo_capa_id
        })
        if existe_elementos_internos:
            raise AplicacionException("El grupo de capas tiene elementos internos, elimínelos primero",
                                      status.HTTP_400_BAD_REQUEST)
        # Se verifica que el grupo de capas no tenga servicios internos.
        existen_servicios_internos: bool = await self._servicio_externo_repositorio.verificar_existencia_por_filtros({
            "estado": ESTADO_ACTIVO,
            "grupo_capa_id": grupo_capa_id
        })
        if existen_servicios_internos:
            raise AplicacionException("El grupo de capas tiene servicios externos, elimínelos primero",
                                      status.HTTP_400_BAD_REQUEST)
        return await self._grupo_capa_repositorio.eliminar(grupo_capa_id, usuario_auditoria_id)
