...
from app.dominio.repositorios.imagen_satelital_repositorio import (
    IImagenSatelitalRepositorio,
)
from app.dominio.repositorios.programacion_repositorio import IProgramacionRepositorio
from app.infraestructura.mongo_db.repositorios.imagen_satelital_repositorio import (
    ImagenSatelitalRepositorio,
)
from app.infraestructura.mongo_db.repositorios.programacion_repositorio import (
    ProgramacionRepositorio,
)
...
class ImagenSatelitalServicio:
    def __init__(
        self,
        programacion_repositorio: IProgramacionRepositorio = Depends(
            ProgramacionRepositorio
        ),
        imagen_satelital_repositorio: IImagenSatelitalRepositorio = Depends(
            ImagenSatelitalRepositorio
        ),
    ):
    ...
    async def descargar(self, programacion_id: str) -> None:
        programacion: ProgramacionEntidad = (
            await self._programacion_repositorio.obtener_por_id(programacion_id)
        )
        try:
            api = SentinelAPI(
                settings.SENTINEL_HUB_USUARIO,
                settings.SENTINEL_HUB_PASSWORD,
                "https://scihub.copernicus.eu/dhus",
            )
            with open(GEOJSON_ARCHIVO, "r") as f:
                geojson_texto = f.read()
            geojson = geojson_to_wkt(json.loads(geojson_texto))
            ...
            api.download_all(
                list(productos), directory_path=settings.CARPETA_IMAGENES_SATELITALES
            )
            ...
            for i in range(len(productos)):
                ...
                if not existe_imagen_satelital:
                    ...
                    folder_salida = os.path.join(
                        settings.CARPETA_IMAGENES_SATELITALES, identificador
                    )
                    if not os.path.exists(folder_salida):
                        os.makedirs(folder_salida)
                    rutas = {}
                    archivo_zip = os.path.join(
                        settings.CARPETA_IMAGENES_SATELITALES, identificador + ".zip"
                    )
                    ...
                    with rasterio.open(
                        os.path.join(folder_salida, identificador + "_RGB.tif"),
                        "w",
                        **perfil,
                    ) as salida:
                    ...
                    perfil.update(count=1, driver="GTiff", dtype=rasterio.float32)
                    np.seterr(divide="ignore", invalid="ignore")
                    # Indice normalizado diferencial de vegetacion.
                    ndvi = np.divide((banda_nir - banda_roja), (banda_nir + banda_roja))
                    with rasterio.open(
                        os.path.join(folder_salida, identificador + "_NDVI.tif"),
                        "w",
                        **perfil,
                    ) as salida:
                        salida.write(ndvi, 1)
                    ...
                    ndwi = np.divide(
                        (banda_verde - banda_nir), (banda_verde + banda_nir)
                    )
                    with rasterio.open(
                        os.path.join(folder_salida, identificador + "_NDWI.tif"),
                        "w",
                        **perfil,
                    ) as salida:
                        salida.write(ndwi, 1)
                    ...
                    for imagen in imagenes:
                        ...
                        geoserver.create_coveragestore(
                            os.path.join(
                                folder_salida, identificador + "_" + imagen + ".tif"
                            ),
            ...
            await self._programacion_repositorio.actualizar(
    ...
    async def obtener_todos(self) -> list[ObtenerTodosImagenSatelitalResponse]:
        imagenes_satelitales: list[
            ImagenSatelitalEntidad
        ] = await self._imagen_satelital_repositorio.obtener_todos(
            {"estado": base_entidad.ESTADO_ACTIVO}
        )
        return [
            ObtenerTodosImagenSatelitalResponse(
    ...
    async def obtener_por_id(
        self, imagen_satelital_id
    ) -> ObtenerPorIdImagenSatelitalResponse:
        imagen_satelital: ImagenSatelitalEntidad = (
            await self._imagen_satelital_repositorio.obtener_por_id(imagen_satelital_id)
        )
        if not imagen_satelital.estado:
            raise AplicacionException(
                "La imagen satelital no existe", status.HTTP_404_NOT_FOUND
            )
        imagen_url: str = self._obtener_imagen_url(imagen_satelital)
        return ObtenerPorIdImagenSatelitalResponse(
            imagen_url=imagen_url, **imagen_satelital.dict()
        )

    async def eliminar(
        self, imagen_satelital_id: str, usuario_auditoria_id: str
    ) -> str:
        existe_imagen_satelital: bool = (
            await self._imagen_satelital_repositorio.verificar_existencia(
                imagen_satelital_id
            )
        )
        if not existe_imagen_satelital:
        ...
        return await self._imagen_satelital_repositorio.eliminar(
            imagen_satelital_id, usuario_auditoria_id
        )
