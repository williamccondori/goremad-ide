...
from app.infraestructura.mongo_db.repositorios.publicacion_repositorio import PublicacionRepositorio
...
class PublicacionServicio:
    ...
    def __init__(self,
                 publicacion_repositorio: IPublicacionRepositorio = Depends(PublicacionRepositorio),
    ...
    async def obtener_todos(self) -> list[ObtenerTodosPublicacionResponse]:
        ...
        return [ObtenerTodosPublicacionResponse(**publicacion.dict()) for publicacion in publicaciones]

    async def crear(self, request: CrearPublicacionRequest, usuario_auditoria_id: str) -> str:
        """
        ...
        existe: bool = await self._publicacion_repositorio.verificar_existencia_por_filtros({
            "estado": base_entidad.ESTADO_ACTIVO,
            "capa": nombre_capa
        })
        if existe:
            raise AplicacionException("La capa ya se encuentra publicada", status.HTTP_400_BAD_REQUEST)
        ...
        try:
            capa_geoserver = geoserver.get_layer(nombre_capa, workspace=request.espacio_trabajo)
        except Exception as excepcion:
            print(excepcion)
            raise AplicacionException("La capa no existe en GeoServer", status.HTTP_404_NOT_FOUND)
