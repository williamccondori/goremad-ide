...
from app.aplicacion.utilidades.wms import obtener_informacion_wms, InformacionWebMapServiceModelo
...
from app.dominio.entidades.servicio_externo_entidad import ServicioExternoEntidad, ServicioExternoCapaEntidad
...
class ServicioExternoServicio:
    ...
    async def __validar_existencia_grupo_capa(self, grupo_capa_id: Optional[str]) -> None:
        """
        ...
        if grupo_capa_id:
            ...
            if not existe_grupo_capa:
                raise AplicacionException("El grupo de capa no existe", status.HTTP_404_NOT_FOUND)
        if grupo_capa_id == "":
            raise AplicacionException("El grupo de capa no puede estar vacío (puede ser NULO)",
    ...
    async def obtener_todos(self, request: ObtenerTodosServicioExternoRequest) \
            ...
            for servicio_externo in servicios_externos:
                if servicio_externo.esta_habilitado:
                    for capa in servicio_externo.capas:
                        servicios_externos_respuesta.append(ObtenerTodosServicioExternoResponse(
                            ...
                            grupo_capa=await self.__obtener_grupo_capa_descripcion(
                                servicio_externo.grupo_capa_id) or "Principal",
    ...
    async def obtener_por_id(self, servicio_externo_id: str) -> ObtenerPorIdServicioExternoResponse:
        """
        ...
        """
        servicio_externo: ServicioExternoEntidad = await self._servicio_externo_repositorio.obtener_por_id(
            servicio_externo_id)
        if not servicio_externo.estado:
            raise AplicacionException("El servicio externo no existe", status.HTTP_404_NOT_FOUND)
    ...
    async def crear(self, request: CrearServicioExternoRequest, usuario_auditoria_id: str) -> str:
        """
        ...
        informacion_wms: InformacionWebMapServiceModelo = obtener_informacion_wms(request.url)
        ...
        servicio_externo: ServicioExternoEntidad = ServicioExternoEntidad(
            ...
            capas=[ServicioExternoCapaEntidad(
                nombre=capa.nombre,
                titulo=capa.titulo,
    ...
    async def actualizar(self, servicio_externo_id: str, request: ActualizarServicioExternoRequest,
        ...
        servicio_externo.capas = [ServicioExternoCapaEntidad(
            nombre=capa.nombre,
            titulo=capa.titulo,
            cuadro_delimitador=capa.cuadro_delimitador
        ) for capa in informacion_wms.capas]
        ...
        return await self._servicio_externo_repositorio.actualizar(servicio_externo_id, servicio_externo)
    ...
    async def eliminar(self, servicio_externo_id: str, usuario_auditoria_id: str) -> str:
        ...
        """
        existe_servicio_externo: bool = await self._servicio_externo_repositorio.verificar_existencia(
            servicio_externo_id)
        if not existe_servicio_externo:
            raise AplicacionException("El servicio externo no existe", status.HTTP_404_NOT_FOUND)
        return await self._servicio_externo_repositorio.eliminar(servicio_externo_id, usuario_auditoria_id)
