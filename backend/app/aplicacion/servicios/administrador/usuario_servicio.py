...
from app.aplicacion.dtos.administrador.actualizar_rol_usuario_request import (
    ActualizarRolUsuarioRequest,
)
from app.aplicacion.dtos.administrador.actualizar_usuario_request import (
    ActualizarUsuarioRequest,
)
from app.aplicacion.dtos.administrador.crear_superusuario_request import (
    CrearSuperusuarioRequest,
)
from app.aplicacion.dtos.administrador.crear_usuario_request import CrearUsuarioRequest
from app.aplicacion.dtos.administrador.obtener_por_id_usuario_response import (
    ObtenerPorIdUsuarioResponse,
)
from app.aplicacion.dtos.administrador.obtener_todos_usuario_response import (
    ObtenerTodosUsuarioResponse,
)
...
class UsuarioServicio:
    ...
    async def obtener_por_id(self, usuario_id: str) -> ObtenerPorIdUsuarioResponse:
        usuario: UsuarioEntidad = await self._usuario_repositorio.obtener_por_id(
            usuario_id
        )
    ...
    async def __validar_email(self, email: str) -> None:
        existe: bool = await self._usuario_repositorio.verificar_existencia_por_filtros(
            {"estado": base_entidad.ESTADO_ACTIVO, "email": email}
        )
        if existe:
    ...
    async def crear(
        ...
        existe = await self._usuario_repositorio.verificar_existencia_por_filtros(
            {"estado": base_entidad.ESTADO_ACTIVO, "username": username}
        )
        if existe:
            raise AplicacionException(
    ...
    async def crear_superusuario(self, request: CrearSuperusuarioRequest) -> str:
        ...
        if not request.llave_secreta == settings.SECRET_KEY:
            raise AplicacionException(
                "La llave secreta no es valida", status.HTTP_403_FORBIDDEN
            )
        # Se valida que no haya más de un superusuario registrado.
        existe = await self._usuario_repositorio.verificar_existencia_por_filtros(
            {"roles": {"$in": [constantes.ROL_SUPER_USUARIO]}}
        )
        if existe:
            raise AplicacionException(
                "Ya existe un superusuario", status.HTTP_400_BAD_REQUEST
            )
        superusuario_id = await self.crear(
            request, request.username, es_superusuario=True
        )
        ...
        configuraciones_id: list[
            str
        ] = await self._configuracion_repositorio.crear_multiple(configuraciones)
    ...
    async def eliminar(self, usuario_id: str, usuario_auditoria_id: str) -> str:
        existe_usuario: bool = await self._usuario_repositorio.verificar_existencia(
            usuario_id
        )
        if not existe_usuario:
            raise AplicacionException("El usuario no existe", status.HTTP_404_NOT_FOUND)
        return await self._usuario_repositorio.eliminar(
            usuario_id, usuario_auditoria_id
        )
    ...
    async def actualizar_rol(
        ...
        if not request.esUsuario:
            raise AplicacionException(
                "El usuario no puede dejar de tener el rol de usuario",
                status.HTTP_400_BAD_REQUEST,
            )
        # Se valida que el usuario tenga al menos un rol.
        if not request.esUsuario and not request.esAdministrador:
            raise AplicacionException(
                "Debe seleccionar al menos un rol", status.HTTP_400_BAD_REQUEST
            )
