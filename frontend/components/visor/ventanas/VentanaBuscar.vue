  ...
  >
    ...
    <a-space direction="vertical" style="width: 100%">
      <a-tabs default-active-key="buscarPorCriterio" type="card">
        <a-tab-pane key="buscarPorCriterio" tab="Buscar por criterio">
          <a-space direction="vertical" style="width: 100%">
            ...
            >
              ...
              <a-button block html-type="submit" icon="search" type="primary">
                Buscar
              </a-button>
            </a-form-model>
            <a-divider />
        ...
        <a-tab-pane key="buscarPorCoordenadas" tab="Buscar por coordenadas">
          <a-space direction="vertical" style="width: 100%">
            <a-form-model
              ref="form2"
              :model="form2"
            ...
            >
              ...
              <a-form-model-item
                :label="form2.proyeccion === 'utm' ? 'Y' : 'Latitud'"
                prop="y"
                :rules="[
                  {
                    required: true,
              ...
              <a-form-model-item
                :label="form2.proyeccion === 'utm' ? 'X' : 'Longitud'"
                prop="x"
                :rules="[
                  {
                    required: true,
                    message: `Ingrese el valor para: ${
                      form2.proyeccion === 'utm' ? 'X' : 'Longitud'
                    }`,
                  },
                ]"
              >
...
export default {
  ...
  methods: {
    ...
    async buscarPorCriterio() {
      const formularioValido = await this.validarFormulario(this.$refs.form1);
    ...
    async buscarPorCoordenada() {
      const formularioValido = await this.validarFormulario(this.$refs.form2);
      if (!formularioValido) return;
      try {
        this.$iniciarCarga();
